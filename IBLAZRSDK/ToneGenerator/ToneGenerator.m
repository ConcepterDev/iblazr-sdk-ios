...
- (id)initWithFrequency:(double)hertz amplitude:(double)volume
{
    if (self = [super init]) {
        ...
        OSStatus result = AudioSessionInitialize(NULL, NULL, NULL, (__bridge void *)(self));
...
- (void)play
{
    //NSLog(@"%@", [NSString stringWithFormat: @"play %.f", frequency]);
    if (!toneUnit)
    {
        [self createToneUnit];
...
- (void)stop
{
    //NSLog(@"%@", [NSString stringWithFormat: @"stop %.f", frequency]);
    if (toneUnit)
    {
